...
export function transferFeedback(error: unknown) {
  ...
  const isMerchantPayment = error instanceof ApiError && error.problem.errorCode === 'MERCHANT_PAYMENT_REQUIRED';
  if (unconfirmed) {
    return {
      ...
      canRetry: true,
      payMerchantLink: false,
    };
  }
  return {
    severity: processing ? 'info' as const : 'error' as const,
    title: processing ? 'Transfer processing' : isMerchantPayment ? 'Merchant payment required' : 'Transfer could not be completed',
    ...
    canRetry: false,
    payMerchantLink: isMerchantPayment,
  };
}
