...
- **Payout (`payouts`)**: An outflow of funds from a `CUSTOMER_WALLET` or `MERCHANT_WALLET` via simulated payout provider clearing (Wallet $\to$ PSP Clearing), secured with balance holds during transit.
