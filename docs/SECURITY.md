...
- Refresh tokens are transmitted via a dedicated HTTP cookie:
  ...
  - **Session Cookie by Default**: Issued without persistent `Max-Age` or `Expires`. Stored in volatile browser session memory and discarded when the browser session terminates (subject to modern browser session-restore/tab re-open configuration). Rotated tokens on `POST /api/auth/refresh` maintain session-cookie scope.
  - Explicit logout clears the cookie with `Max-Age = 0` and revokes the active token in the database.
  - Server-side token maximum validity in PostgreSQL is enforced via `expires_at` (default 7 days).
...
- **Silent Session Restoration**: On initial application load or hard reload, the frontend calls `POST /api/auth/refresh` with `credentials: 'include'`. If a valid `ledgerguard_refresh_token` session cookie is present, a fresh access token is loaded into memory without user interaction.
