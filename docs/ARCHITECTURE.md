...
- **Notification Worker & Idempotent Consumer Inbox (Phase 18)**: Standalone `notification-worker` service consumes `ledgerguard.domain-events.v1` with its own dedicated `notification_worker` database (Flyway `V1__create_notification_worker_tables.sql`). It enforces database-backed idempotent inbox deduplication via `processed_events` using atomic `INSERT ... ON CONFLICT (event_id) DO NOTHING` claims. The durable business side effect `notification_deliveries` (unique `event_id`, status `DELIVERED` denoting that a durable notification worker record was created) commits inside the *exact same PostgreSQL database transaction* as the claim. At most one durable delivery row is committed per event ID (exactly one for successfully processed events, zero for DLT-routed events). Duplicate Kafka deliveries skip side effects cleanly. Poison and unsupported messages are routed via `DefaultErrorHandler` and `DeadLetterPublishingRecoverer` (configured with `failIfSendResultIsError=true` and bounded wait timeout) to `ledgerguard.domain-events.v1.DLT` without blocking partition consumption.
...
- **Database & Domain Isolation**: `notification-worker` connects strictly to `notification_worker` database and never accesses `ledgerguard` financial schema. Wire contracts use pure JSON without shared Java entity classes.
