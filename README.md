...
- **Current State:** Phase 23 Completed — External State Machines & Ambiguous Outcome Handling: Implemented a formal six-state lifecycle (`CREATED → PROCESSING → UNKNOWN → RECONCILIATION_REQUIRED → SUCCEEDED/FAILED`) for `funding_operations` and `payouts` enforced by Flyway V13 migration triggers. Key features: atomic `CREATED → PROCESSING` submission claim (at-most-one provider POST via pessimistic `FOR UPDATE` row lock); RFC-9457 ProblemDetail `type` URI classification distinguishing definite pre-acceptance rejections (`urn:ledgerguard:psp:error:temporary-failure` → `FAILED`) from ambiguous outcomes (generic 500/timeout → `UNKNOWN`); durable background status poller with four-phase cycle (Step 0 exhaustion finalizer, Step A `SKIP LOCKED` claim, Step B non-transactional provider GET, Step C settlement); payout hold protection extended to `UNKNOWN` and `RECONCILIATION_REQUIRED`; late webhook recovery (`RECONCILIATION_REQUIRED` → `SUCCEEDED/FAILED`); 8 new lifecycle test classes (20 tests), workspace total 486 tests with 0 failures, 0 errors.
- **Next Step:** Phase 24 — External Reconciliation Engine & Provider Settlement Reports.
